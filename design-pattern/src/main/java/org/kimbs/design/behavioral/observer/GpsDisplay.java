package org.kimbs.design.behavioral.observer;

public class GpsDisplay implements Observer {

    private final GpsController gpsController;

    public GpsDisplay(GpsController gpsController) {
        this.gpsController = gpsController;
    }

    @Override
    public void update() {
        Position position = gpsController.getPosition();
        System.out.printf("[Display] current position. latitude: %.15f, longitude: %.15f\n", position.getLatitude(), position.getLongitude());
    }
}
